...
for eval_file in eval_file_list:
    ...
    label_list = np.loadtxt(args.fsd_class_labels, delimiter=',', dtype=str, skiprows=1)
    ...
    if os.path.exists(save_cache_path) == True:
        ...
        if text_embed_setting == 'gpt':
            ...
            if llm_task == 'cla':
                list_of_preds_for_cache = [x['pred'].split(': ')[-1].split('; ') for x in eval_data]
