...
for eval_file in eval_file_list:
    ...
    label_csv = args.as_class_labels
    ...
    if os.path.exists(label_embed_cache_file):
        ...
        if text_embed_setting == 'gpt':
            gpt_embeddings_list = get_gpt_embedding(['sound of ' + ori_label_dict[key].lower() for key in ori_label_dict.keys()])
        for key_idx,key in enumerate(ori_label_dict.keys()):
            class_name = ori_label_dict[key]
            # print(class_name)
            if text_embed_setting == 'gpt':
                # label_dict[class_name] = get_gpt_embedding('sound of ' + class_name.lower())
                label_dict[class_name] = gpt_embeddings_list[key_idx]
    ...
    if os.path.exists(save_cache_path) == True:
        ...
        if text_embed_setting == 'gpt':
            ...
            if llm_task == 'cla':
                list_of_preds_for_cache = [x['pred'].split(':')[-1].split(': ')[-1].split('; ') for x in eval_data]
