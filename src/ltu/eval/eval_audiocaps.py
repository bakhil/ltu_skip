...
for eval_file in eval_file_list:
    ...
    for trial in range(5):
        ...
        res = eval_metrics.evaluate_metrics(save_fold + '/sample_pred_audiocaps_{:d}.csv'.format(trial), save_fold + '/sample_truth_audiocaps_{:d}.csv'.format(trial), 5)
    ...
    np.savetxt(save_fold + '/all_acaps_summary_ablation.csv', result_summary, delimiter=',', fmt='%s')
    ...
    np.savetxt(save_fold + '/summary_audiocaps_{:s}.csv'.format(time_string), result_summary, delimiter=',', fmt='%s')