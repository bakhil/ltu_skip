...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, LlamaModel) or isinstance(module, LlamaModel_jump):
...
class LlamaModel_jump(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        pre_seq_length = seq_length
        seq_length = audio_length + seq_length # [B, length, 1280]

        seq_length_with_past = seq_length
        pre_seq_length_with_past = pre_seq_length
        
        past_key_values_length = 0
        pre_past_key_values_length = 0
        if past_key_values is not None:
            past_key_values_length = past_key_values[-1][0].shape[2]
            pre_past_key_values_length = past_key_values[0][0].shape[2]
            seq_length_with_past = seq_length_with_past + past_key_values_length
            pre_seq_length_with_past = pre_seq_length_with_past + pre_past_key_values_length
        ...
        if pre_position_ids is None:
            device = input_ids.device if input_ids is not None else inputs_embeds.device
            pre_position_ids = torch.arange(
                pre_past_key_values_length, pre_seq_length + pre_past_key_values_length, dtype=torch.long, device=device
            )
            pre_position_ids = pre_position_ids.unsqueeze(0).view(-1, pre_seq_length)
        else:
            pre_position_ids = pre_position_ids.view(-1, pre_seq_length).long()

        # convert id to embedding
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)
        if audio_input != None:
            ###### Add the audio embeddings at the insertion layer
        ...
        pre_attention_mask = self._prepare_decoder_attention_mask(
            pre_attention_mask, (batch_size, pre_seq_length), inputs_embeds, pre_past_key_values_length
        )
        ...
        attention_mask_to_layer = pre_attention_mask
        position_ids_to_layer = pre_position_ids
        for idx, decoder_layer in enumerate(self.layers):
            if idx == jump_to_layer:
                # breakpoint()
                attention_mask_to_layer = attention_mask
                position_ids_to_layer = position_ids
                if audio_input != None:
                    hidden_states = torch.concat([audio_input, hidden_states], dim=1)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    hidden_states,
                    attention_mask_to_layer,
                    position_ids_to_layer,
                    None,
                )
            else:
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=attention_mask_to_layer,
                    position_ids=position_ids_to_layer,
                    past_key_value=past_key_value,
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                )
...
class LlamaForCausalLM_jump(LlamaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModel_jump(config)
    ...
    def prepare_inputs_for_generation(
        ...
        if attention_mask is not None:
            pre_attention_mask = attention_mask[:, -input_ids.shape[1]:]
        ...
        if attention_mask is not None and position_ids is None:
            ...
            pre_position_ids = pre_attention_mask.long().cumsum(-1) - 1
            pre_position_ids.masked_fill_(pre_attention_mask == 0, 1)
            if past_key_values:
                position_ids = position_ids[:, -1].unsqueeze(-1)
                pre_position_ids = pre_position_ids[:, -1].unsqueeze(-1)
        ...
        model_inputs.update(
            {
                ...
                "attention_mask": attention_mask,
                "jump_to_layer": kwargs.get("jump_to_layer", 0),
                "pre_attention_mask": pre_attention_mask,
                "pre_position_ids": pre_position_ids,
            }
        )
